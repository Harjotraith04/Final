...
const ProjectSettings = ({ projectId, open: externalOpen, onClose: externalOnClose }) => {
  ...
  const fetchProjectDetails = async () => {
    // Don't fetch if projectId is not a valid integer (e.g., "default" fallback)
    if (!projectId || isNaN(parseInt(projectId))) {
      console.log('ProjectSettings: Invalid or missing projectId, skipping API call');
      setLoading(false);
      setError('Please select a valid project to view settings');
      return;
    }
  ...
  const handleClose = () => {
    ...
    if (!projectId || isNaN(parseInt(projectId))) {
      setError('Please select a valid project to add collaborators');
      return;
    }
    ...
    if (!projectId || isNaN(parseInt(projectId))) {
      setError('Please select a valid project to remove collaborators');
      return;
    }
  ...
  const handleUpdateProjectDetails = async () => {
    if (!projectId || isNaN(parseInt(projectId))) {
      setError('Please select a valid project to update');
      return;
    }
  ...
  return (
    <>
      {/* Conditionally render the button only if onClose is not provided (standalone usage) */}
      {!externalOnClose && (
