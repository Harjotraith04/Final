...
class CodeOutput(BaseModel):
    ...
    confidence: int = Field(
        ...
        ge=0, le=100)
    color: str = Field(
        description="The color assigned to the code, represented as a hex value (e.g., #FF5733)."
    )
...
class ReportOutput(BaseModel):
    """
    Represents the output of the report generation process.
    """
    report_text: str = Field(
        description="A well organized report about the provided codes and themes for the purpose of thematic analysis.")
