...
class CodeAssignmentService:
    ...
    def bulk_code_assignment(
        ...
        try:
            ...
            document_map = {int(doc.id): doc for doc in documents} # type: ignore
            ...
            for doc_id in document_ids:
                ...
                if document.owner_id != user_id and user_id not in document.collaborator_ids:
                    raise ValueError(
                        f"Access denied: User {user_id} is not authorized to access document {doc_id}"
                    )
    ...
    def submit_assignments(
        ...
        """Submit code assignments and return compiled Submited assignments and codes"""
        ...
        # Get assignments and validate access
        assignments = db.query(CodeAssignmentModel).filter(
        ...
        # Update status to 'pending' and mark as submitted
        for assignment in assignments:
            assignment.status = "pending"  # type: ignore
            assignment.is_submitted = True  # type: ignore
            assignment.updated_at = datetime.datetime.now(datetime.timezone.utc)  # type: ignore
            db.add(assignment)

        db.commit()

        # Compile finalized assignments
        ...
        # Compile unique codes used in the finalized assignments
        unique_codes = {
            a.code.id: {
                "id": a.code.id,
                "name": a.code.name,
        ...
        return {
            ...
            "message": f"Successfully finalized {len(submitted_assignments)} assignments"
        }
