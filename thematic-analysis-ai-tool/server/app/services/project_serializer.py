...
class ProjectSerializer:
    ...
    def serialize_code(code, user_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "is_owner": code.created_by_id == user_id  # Add flag to identify if current user owns this code
        }

    @staticmethod
    def get_user_codebooks(db: Session, all_codes: List[Code], user_id: int, project_id: int) -> List[Dict[str, Any]]:
        """Get codebooks used by codes in the project"""
        # Collect unique codebook IDs from all codes
        codebook_ids = set()
        ...
        for codebook in codebooks:
            # Get all codes in this codebook for the project
            all_codes_in_codebook = [
                c for c in codebook.codes
                if c.project_id == project_id
            ]
            
            # Get user-specific codes in this codebook
            user_codes_in_codebook = [
                c for c in all_codes_in_codebook
            ...
            result.append({
                ...
                "total_codes_count": len(all_codes_in_codebook)
            })
