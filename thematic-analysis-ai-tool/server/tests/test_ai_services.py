...
Overall, the healthcare experience was described as overwhelming."""
    ...
    def test_codebook(self, client, auth_headers, test_project):
        ...
        print(f"Debug: Creating codebook with project_id={project_id}")
            ...
            with patch('app.services.codebook_service.CodebookService.get_or_create_ai_session_codebook') as mock_codebook:
                ...
                with patch('app.services.code_service.CodeService.create_code') as mock_create_code:
                    ...
                    with patch('sqlalchemy.orm.Session.bulk_insert_mappings') as mock_bulk_insert:
                        ...
                        # Assertions for initial coding response structure
    ...
    def test_deductive_coding_endpoint(self, mock_llm_service, client, auth_headers, test_document, test_codebook, test_project):
        ...
        with patch('app.services.code_assignment_service.CodeAssignmentService.bulk_code_assignment') as mock_bulk_assignment:
            ...
            with patch('app.services.codebook_service.CodebookService.get_or_create_ai_session_codebook') as mock_codebook:
                ...
                # Assertions for deductive coding response structure
                assert isinstance(result, dict)
                assert "codebook_ids" in result
                assert result["codebook_ids"] == [test_codebook["id"]]
                assert "results" in result
                assert "summary" in result

                print(f"Deductive coding result: {result}")

    @patch('app.services.ai.ai_coding_service.AICodingService.generate_themes')
    @patch('sqlalchemy.orm.Query.filter')
    def test_generate_themes_endpoint(self, mock_filter, mock_generate_themes, client, auth_headers, test_codebook):
        """Test the theme generation API endpoint"""

        # Mock code assignments that would be returned from the database
        mock_code = MagicMock()
        mock_code.project_id = test_codebook["project_id"]

        mock_document = MagicMock()

        mock_assignment1 = MagicMock()
        mock_assignment1.code_id = 1
        mock_assignment1.document_id = 101
        mock_assignment1.text_snapshot = "Sample text 1"
        mock_assignment1.code = mock_code

        mock_assignment2 = MagicMock()
        mock_assignment2.code_id = 2
        mock_assignment2.document_id = 102
        mock_assignment2.text_snapshot = "Sample text 2"
        mock_assignment2.code = mock_code

        # Set up nested mocks for the database query
        mock_query_instance = MagicMock()
        mock_filter.return_value = mock_query_instance
        mock_query_instance.options.return_value = mock_query_instance
        mock_query_instance.all.return_value = [
            mock_assignment1, mock_assignment2]

        # Mock direct response from service
        mock_theme_response = [{
            "id": 1,
            "name": "Healthcare System Challenges",
            "description": "Overarching challenges in healthcare delivery and patient experience",
            "project_id": test_codebook["project_id"],
            "user_id": 1,
            "created_at": "2025-06-29T12:00:00",
            "reasoning": "These codes relate to healthcare challenges",
            "related_codes": ["Communication Issues", "Emotional Response", "Access Barriers"]
        }]

        # Set the return value for the mocked service regardless of input parameters
        mock_generate_themes.return_value = mock_theme_response

        # Test the endpoint with code assignment IDs instead of full objects
        request_data = {
            "code_assignment_ids": [1, 2]
        }
        ...
        # For now, we'll check that the response isn't a server error
        # It might return 400 if validation fails which is acceptable during testing
    ...
    def test_ai_endpoints_authentication(self, client):
        ...
        endpoints = [
            ...
            ("/api/v1/ai/deductive-coding",
             {"document_ids": [1], "codebook_ids": [1]}),
            ("/api/v1/ai/generate-themes", {"code_assignment_ids": [1]})
        ]
    ...
    def test_ai_endpoints_validation(self, client, auth_headers):
        ...
        test_cases = [
            ...
            # Missing codebook_ids
            ("/api/v1/ai/deductive-coding", {"document_ids": [1]}),
            ("/api/v1/ai/generate-themes", {}),  # Missing code_assignment_ids
        ]
