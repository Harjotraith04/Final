...
def test_project_comprehensive_includes_collaborators(client, auth_headers):
    ...
    # Create a finalized codebook for the project
    codebook_data = {
        "name": "Finalized Codebook",
        "description": "A finalized codebook for testing",
        "project_id": project_id,
        "finalized": True
    }

    codebook_response = client.post(
        "/api/v1/codebooks/", json=codebook_data, headers=auth_headers)
    assert codebook_response.status_code == 201
